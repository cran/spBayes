spPredict <- function(sp.obj, pred.coords, pred.covars, start=1, end, thin=1, verbose=TRUE, ...){
  ...
  if(!class(sp.obj) %in% c("spGGT","bayesGeostatExact","spLM","spMvLM")){
    stop("error: requires an output object of class spGGT, bayesGeostatExact, spLM, or spMvLM\n")
  }
  ...
  if(!obj.class %in% c("spGGT", "bayesGeostatExact", "spLM", "spMvLM"))
    stop("error: requires an output object of class spGGT, bayesGeostatExact, or spMvLM\n")
  ...
  if(obj.class == "spMvLM"){
   ...
   # }
    ...
    ##
    ##Always ridge the no nugget model
    ##
    if(!nugget && is.pp){
      L <- rep(diag(sqrt(0.01))[lower.tri(diag(m), diag=TRUE)], n)
      nugget <- TRUE
    }
