...
extern "C" {
  ...
  SEXP pPCovInvDet_wrap(SEXP knotsD_r, SEXP coordsKnotsD_r, SEXP C_r, SEXP C_str_r, SEXP ct_r, 
			SEXP n_r, SEXP m_r, SEXP tauSq_r, SEXP sigmaSq_r, SEXP theta_r, SEXP tmp_mm_r, SEXP tmp_nm_r,
			SEXP covModel_r, SEXP brute){
    int nPramPtr = 1;
    ...
    REAL(logDet_r)[0] = pPCovInvDet(REAL(knotsD_r), REAL(coordsKnotsD_r), REAL(C_r), REAL(C_str_r), REAL(ct_r), 
				    INTEGER(n_r)[0], INTEGER(m_r)[0], REAL(tauSq_r)[0], REAL(sigmaSq_r)[0], REAL(theta_r), 
				    REAL(tmp_mm_r), REAL(tmp_nm_r),
				    covModel, nPramPtr, covModelObj, cov1ParamPtr, cov2ParamPtr, INTEGER(brute)[0]);
