...
extern "C" {
  ...
  SEXP spDynLM(SEXP Y_r, SEXP X_r, SEXP p_r, SEXP n_r, SEXP Nt_r, SEXP coordsD_r,
	       ...
	       SEXP phiTuning_r, SEXP nuTuning_r, SEXP covModel_r, SEXP nSamples_r, SEXP missing_r, SEXP verbose_r, SEXP nReport_r){
    ...
    *****************************************/ 
    ...
    double *ySamples = REAL(ySamples_r);
    ...
    *****************************************/
    ...
    for(s = 0; s < nSamples; s++){
      ...
      //Sigma_eta^{-1}
      F77_NAME(dpotrf)(lower, &p, sigmaEta, &p, &info); if(info != 0){error("c++ error: dpotrf failed\n");}
     ...
     for(t = 0, j = 0; t < Nt; t++){
       ...
       for(i = 0; i < n; i++, j++){
     	 if(missing[j]==1){
     	   Y[n*t+i] = ySamples[s*Ntn+t*n+i] = rnorm(tmp_n3[i] + u[n*t+i], sqrt(theta[t*nTheta+tauSqIndx]));
     	 }else{
	 ...
	 for(i = 0; i < p; i++){
       ...
       for(i = 0; i < n; i++) logDet += 2*log(C[i*n+i]); //for phi
       F77_NAME(dpotri)(lower, &n, C, &n, &info); if(info != 0){error("c++ error: dpotri failed\n");}

       F77_NAME(dcopy)(&nn, C, &incOne, C3, &incOne);//save for sigma^2 and phi
       ...
       if(t == 0){
         ...
         if(covModel == "matern"){
       	 ...
       	 F77_NAME(dpotrf)(lower, &n, C2, &n, &info); if(info != 0){error("c++ error: dpotrf failed\n");}
       	 ...
       	 F77_NAME(daxpy)(&nn, &one, C2, &incOne, C, &incOne);
         ...
         if(covModel == "matern"){
       	 ...
       	 F77_NAME(dsymv)(lower, &n, &one, C3, &n, &u[(t-1)*n], &incOne, &zero, tmp_n, &incOne);
       	 F77_NAME(dsymv)(lower, &n, &one, C2, &n, &u[(t+1)*n], &incOne, &one, tmp_n, &incOne);
       	 ...
       	 F77_NAME(dsymv)(lower, &n, &one, C, &n, &u[(t-1)*n], &incOne, &zero, tmp_n, &incOne);
       ...
       F77_NAME(dsymv)(lower, &n, &one, C3, &n, tmp_n, &incOne, &zero, tmp_n2, &incOne);
       
       theta[t*nTheta+sigmaSqIndx] = 1.0/rgamma(sigmaSqIG[t*2]+n/2.0, 
       						1.0/(sigmaSqIG[t*2+1]+0.5*F77_NAME(ddot)(&n, tmp_n, &incOne, tmp_n2, &incOne)));
       ...
       logDetCand = 0;
       F77_NAME(dpotrf)(lower, &n, C, &n, &info); if(info != 0){error("c++ error: dpotrf failed\n");}
       for(i = 0; i < n; i++) logDetCand += 2*log(C[i*n+i]); //for phi
       F77_NAME(dpotri)(lower, &n, C, &n, &info); if(info != 0){error("c++ error: dpotri failed\n");}
