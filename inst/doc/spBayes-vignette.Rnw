...
$-log(0.05)/\phi$. The decay parameters in $\bphi$ provide an
...
form of Model 5, allowing response specific spatial decay terms,
\[
...
and $m$ spatial decay parameters $\bphi$.  The code block below
...
\begin{table}[!h]
\begin{center}
\caption{Percentiles of the posterior distributions of the parameters in Model 6.  $\beta$ subscripts refer to the response variable and parameter, respectively.  Subscripts on $\bK$ and $\Psi$ refer to the covariance matrix element.  Subscripts on the spatial decay parameters, $\phi$, refer to the response variable. Summaries generated from three chains of 4,000 samples.} \label{SyntheticEsts}
\begin{tabular}
[c]{|c|c|}%
...
\end{verbatim}
The inverse--Wishart prior is used for both cross--covariance matrices.  The empirical semivariograms suggest that the nugget and partial sill are about equal for each response variable (i.e., $\sim100$ for basal area and $\sim2600$ for biomass).  The vague scale matrix defined for the common inverse--Wishart centers each response on the suggested values but does not impose any off--diagonal association.  The noninformative prior on the spatial decay parameters corresponds to an interval of 100 to 2,000 meters.
...
\begin{table}[h!]
\begin{center}
\caption{Percentiles of the posterior distribution of model parameters.  $\beta$ subscripts refer to the response variable and parameter, respectively.  Subscripts on $\bK$ and $\Psi$ refer to the covariance matrix element.  Subscripts on the spatial decay parameters, $\phi$, refer to the response variable model. Summaries generated from six chains of 9,000 samples.} \label{RealEsts}
\begin{tabular}
[c]{|c|c|}%
\hline Parameter & Estimates: 50\% (2.5\%, 97.5\%)\\\hline
